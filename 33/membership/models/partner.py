...
class Partner(models.Model):
    ...
    def _cron_reset_expired_pricelists(self):
        default_pricelist = self.env.ref('product.list0')  # Fallback default
        expired_partners = self.search([
            ('membership_stop', '<', fields.Date.today()),
            '|',
            ('property_product_pricelist', '!=', default_pricelist.id),
            ('property_product_pricelist', '!=', False)
        ])
        for partner in expired_partners:
            # Use the non-member pricelist from the membership product (if any)
            membership_products = partner.member_lines.mapped('membership_id')
            non_member_pricelist = membership_products[:1].non_member_pricelist_id or default_pricelist
            partner.sudo().write({
                'property_product_pricelist': non_member_pricelist.id
            })