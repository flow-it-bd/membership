...
class MembershipLine(models.Model):
    ...
    def unlink(self):
        # Track partners and their deleted products' non-member pricelists
        partner_deleted_data = {}
        for line in self:
            partner = line.partner
            product = line.membership_id
            if partner and product:
                if partner not in partner_deleted_data:
                    partner_deleted_data[partner] = {
                        'non_member_pricelists': [],
                        'deleted_products': []
                    }
                partner_deleted_data[partner]['non_member_pricelists'].append(product.non_member_pricelist_id)
                partner_deleted_data[partner]['deleted_products'].append(product)

        # Delete the membership lines
        res = super().unlink()

        # Process each partner
        for partner, data in partner_deleted_data.items():
            # Check remaining membership lines (sorted by end date)
            remaining_lines = partner.member_lines.sorted(
                lambda l: l.date_to or l.date,
                reverse=True
            )
            if remaining_lines:
                # Use the latest remaining membership's PAID pricelist
                latest_membership = remaining_lines[0].membership_id
                new_pricelist = latest_membership.membership_pricelist_id
            else:
                # Use the last deleted product's NON-MEMBER pricelist
                new_pricelist = data['non_member_pricelists'][-1] if data['non_member_pricelists'] else None

            # Update partner's pricelist
            if new_pricelist:
                partner.sudo().write({
