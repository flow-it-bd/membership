...
class MembershipLine(models.Model):
    ...
    def unlink(self):
        # Map partners to their non-member pricelist (from the deleted line's product)
        partner_pricelist = {}
        for line in self:
            partner = line.partner
            membership_product = line.membership_id
            if partner and membership_product:
                # Get the non-member pricelist from the membership product
                non_member_pricelist = membership_product.non_member_pricelist_id
                partner_pricelist[partner] = non_member_pricelist

        # Delete the membership lines
        res = super().unlink()

        # Update the partner's pricelist
        for partner, pricelist in partner_pricelist.items():
            partner.sudo().write({
